...
namespace exchangecore\progressencode;
...
class Encoder {
    ...
    public static function encode($inputString)
    {
        ...
        for ($i = 0; $i < 16; $i++) {
            ...
            if ((chr($lower) >= 'A' && chr($lower) <= 'Z')
                || (chr($lower) >= 'a' && chr($lower) <= 'z')) {
                $target[$i] = $lower;
            } else {
        ...
        return implode('', array_map('chr', $target));
    }

    private static function hash($scratch, $hash)
    {
        for ($i = 15; $i >= 0; $i--) {
            $hash = ($hash >> 8 ^ self::$lookup[$hash & 0xff] ^ self::$lookup[$scratch[$i]]);
        }
